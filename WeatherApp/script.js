...
async function updateForecastsInfo(city) {
    ...
    const timeTaken = ['06:00:00', '12:00:00', '18:00:00'];
    const todayDate = new Date().toISOString().split('T')[0];
    const processedDates = new Set(); // To track which dates have already been processed

    forecastItemsContainer.innerHTML = '';

    forecastsData.list.forEach(forecastWeather => {
        const [forecastDate, forecastTime] = forecastWeather.dt_txt.split(' '); 
        if (timeTaken.includes(forecastTime) && forecastDate !== todayDate && !processedDates.has(forecastDate)) {
...
function updateForecastsItems(weatherData) {
    ...
    const forecastItem = `    
        <div class="forecast-item">
            ...
            <img src="assets/weatherIcon/${getWeatherIcon(id)}"  class="forecast-item-img">
